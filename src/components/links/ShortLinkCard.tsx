...
export function ShortLinkCard({ link, onUpdate, onDelete }: ShortLinkCardProps) {
  ...
  const handleDelete = async () => {
    if (!confirm(`Are you sure you want to delete the link "${link.code}"? This cannot be undone.`)) {
      return
    }
  ...
  const getDestinationBadge = () => {
    const badges = {
      INTAKE_FORM: { label: 'Tax Intake', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' },
      CONTACT_FORM: { label: 'Contact Form', color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' },
      CUSTOM: { label: 'Custom URL', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' },
    }
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 border rounded-lg overflow-hidden transition-all ${
      link.isActive ? 'border-gray-200 dark:border-gray-800' : 'border-gray-300 dark:border-gray-700 opacity-60'
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <Link2 className={`w-5 h-5 ${link.isActive ? 'text-primary-600' : 'text-gray-400'}`} />
