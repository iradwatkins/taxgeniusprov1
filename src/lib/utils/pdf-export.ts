    ...
    for (const section of params.sections) {
      try {
        ...
        if (section.type === 'metrics') {
          try {
            if (!section.data?.metrics || !Array.isArray(section.data.metrics)) {
...
export async function exportMaterialsReport(params: {
  ...
  const avgConversionRate = materials.length > 0
  ...
  const sections: ReportSection[] = [
    {
      ...
      data: {
        metrics: [
          ...
          { label: 'Stage 2: Intake Started', value: funnelData.stage2_intakeStarts.toLocaleString() },
          { label: 'Stage 3: Intake Completed', value: funnelData.stage3_intakeCompletes.toLocaleString() },
          { label: 'Stage 4: Return Filed', value: funnelData.stage4_returnsFiled.toLocaleString() }
        ]
      }
    },
    {
 ...
 */
export function generateFunnelInsights(funnelData: any): string {
  const insights: string[] = []

  if (funnelData.dropoff.clickToStart > 70) {
    insights.push('• High drop-off between click and intake start (>70%). Consider simplifying the onboarding process or reducing friction.')
  }

  if (funnelData.dropoff.startToComplete > 60) {
    insights.push('• Significant drop-off during intake form completion (>60%). Form may be too long or complex. Consider breaking into smaller steps.')
  }

  if (funnelData.dropoff.completeToFiled > 50) {
    insights.push('• High abandonment after form completion (>50%). Follow up with users to complete their tax return filing.')
  }
  ...
  if (funnelData.conversionRates.overallConversion > 15) {
    ...
    insights.push('• Low overall conversion rate (<5%). Review entire customer journey for optimization opportunities.')
  }
