...
export function exportFunnel(funnelData: FunnelData[], dateRange: string = 'all'): void {
  const columns: ExportColumn[] = [
    ...
    { key: 'dropOffRate', label: 'Drop-off Rate', formatter: (val) => val ? `${val.toFixed(1)}%` : 'N/A' },
    { key: 'conversionRate', label: 'Stage Conversion Rate', formatter: (val) => val ? `${val.toFixed(1)}%` : 'N/A' }
  ]
...
export function exportSourceBreakdown(sourceData: SourceData[], breakdownType: string = 'all'): void {
  const columns: ExportColumn[] = [
