...
async function saveChunk(sessionId: string, chunkIndex: number, chunkData: Buffer): Promise<void> {
  ...
  logger.info(`Saved chunk ${chunkIndex} to ${chunkPath} (${chunkData.length} bytes)`);
}
...
async function mergeChunks(
  ...
  logger.info(`Merging ${totalChunks} chunks for session ${sessionId}`);
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    logger.info(`Read chunk ${i}: ${chunkData.length} bytes`);
  }

  // Concatenate all chunks
  const finalBuffer = Buffer.concat(chunks);
  logger.info(`Merged ${chunks.length} chunks into ${finalBuffer.length} bytes`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isLastChunk) {
      logger.info(`Last chunk received for session ${sessionId}, checking completion...`);
      ...
      try {
        ...
        logger.info(`Successfully merged file: ${fileName} (${mergedFile.length} bytes)`);
