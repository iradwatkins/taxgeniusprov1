...
import { generateCompleteCityContent, type CityData, type TaxServiceSpec } from './city-content-prompts';
import { OllamaClient } from '../integrations/ollama/ollama-client';
import { generateCityTaxImage, type ImageGenerationResult } from '../integrations/google-imagen/imagen-client';
...
export async function generate200CityPages(
  ...
  for (let i = 0; i < cities.length; i += batchSize) {
    ...
    logger.info(`[SEO Brain] Processing batch ${Math.floor(i / batchSize) + 1} (cities ${i + 1}-${Math.min(i + batchSize, cities.length)})...`);
  ...
  logger.info(`[SEO Brain] Campaign ${campaignId} complete: ${generated} generated, ${failed} failed`);
  ...
  try {
    ...
    if (generateImages) {
      try {
        ...
        logger.error(`[SEO Brain]   Image generation failed for ${city.name}, continuing without image`);
      }
    }
