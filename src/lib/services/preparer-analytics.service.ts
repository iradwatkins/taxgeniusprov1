 ...
 */
export async function getPreparerDashboardStats(preparerId: string): Promise<PreparerDashboardStats> {
  try {
    const now = new Date()
 ...
 */
export async function getPreparerTopReferrers(preparerId: string, limit: number = 10): Promise<TopReferrer[]> {
  try {
    ...
    const topReferrers: TopReferrer[] = referrers.map(referrer => {
      ...
      const completedReferrals = referrer.referrerReferrals.filter(r => r.status === 'COMPLETED').length
 ...
 */
export async function getPreparerTopLinks(preparerId: string, limit: number = 10): Promise<PreparerLinkPerformance[]> {
  try {
 ...
 */
export async function getTopPreparers(metric: 'clients' | 'returns' | 'revenue' = 'clients', limit: number = 10) {
  try {
    ...
    const preparerStats = preparers.map(preparer => {
      ...
      preparer.preparerClients.forEach(pc => {
        const returns = pc.client.taxReturns.filter(tr =>
          tr.status === 'FILED' || tr.status === 'ACCEPTED'
        )
