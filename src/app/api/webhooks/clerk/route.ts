...
import { findLeadByEmail, convertLeadToClient } from '@/lib/services/lead-conversion.service';
 ...
 * Auto-convert leads to CLIENT profiles
 */
async function handleUserCreated(evt: WebhookEvent) {
  ...
  logger.info(`🆕 New user created: ${clerkUserId} (${primaryEmail})`);

  try {
    // PRIORITY 1: Check if this user filled out tax intake form
    if (primaryEmail) {
      const lead = await findLeadByEmail(primaryEmail);

      if (lead && !lead.convertedToClient) {
        logger.info(
          `📋 Found tax intake lead for ${primaryEmail}, auto-converting to CLIENT profile`
        );
        ...
        if (result.success) {
          ...
          // Continue to normal flow if conversion fails
        }
      }
    }

    // PRIORITY 2: Normal flow - wait for role selection
