...
import { calculateFederalTax, formatCurrency, formatPercentage, FilingStatus, TaxCalculationResult } from '@/lib/tax-data/tax-calculator';
                ...
                <CardContent>
                  <form onSubmit={handleCalculate} className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={taxYear.toString()} onValueChange={(value) => setTaxYear(parseInt(value) as 2024 | 2025)}>
                    ...
                    <div className="space-y-4 pt-4 border-t">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="otherIncome">Other Income (Interest, Dividends, Capital Gains)</Label>
                        <Input
                          id="otherIncome"
                          type="number"
                          placeholder="0"
                    ...
                    <div className="space-y-4 pt-4 border-t">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="adjustments">Adjustments to Income (IRA, Student Loan Interest, etc.)</Label>
                        <Input
                          id="adjustments"
                          type="number"
                          placeholder="0"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="itemizedDeductions">Itemized Deductions (Optional - leave blank for standard)</Label>
                        <Input
                    ...
                    <div className="space-y-4 pt-4 border-t">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="withholding">Federal Tax Already Withheld (From W-2 Box 2)</Label>
                        <Input
                          id="withholding"
                          type="number"
                          placeholder="0"
            ...
            >
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <Card className="text-center border-2">
                  ...
                  <CardContent>
                    <p className="text-3xl font-bold text-foreground">{formatCurrency(results.totalTaxLiability)}</p>
                  </CardContent>
                </Card>
                ...
                <Card className="text-center border-2">
                  ...
                  <CardContent>
                    <p className="text-3xl font-bold text-foreground">{formatPercentage(results.effectiveRate)}</p>
                  </CardContent>
                </Card>
                ...
                <Card className="text-center border-2">
                  <CardHeader>
                    ...
                    <CardTitle className="text-sm font-medium text-muted-foreground">Total Tax Credits</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-3xl font-bold text-success">{formatCurrency(results.totalCredits)}</p>
                  </CardContent>
                </Card>

                {/* Refund or Owed */}
                <Card className={`text-center border-2 ${results.refundOrOwed >= 0 ? 'border-success/50 bg-success/5' : 'border-destructive/50 bg-destructive/5'}`}>
                  <CardHeader>
                    <div className={`w-16 h-16 ${results.refundOrOwed >= 0 ? 'bg-success/10' : 'bg-destructive/10'} rounded-full flex items-center justify-center mx-auto mb-2`}>
                      <TrendingUp className={`w-8 h-8 ${results.refundOrOwed >= 0 ? 'text-success' : 'text-destructive'}`} />
                    </div>
                  ...
                  <CardContent>
                    <p className={`text-3xl font-bold ${results.refundOrOwed >= 0 ? 'text-success' : 'text-destructive'}`}>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex justify-between items-center pt-2">
                      ...
                      <span className="font-bold text-primary">{formatCurrency(results.taxableIncome)}</span>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="space-y-3">
                    {results.bracketBreakdown.map((bracket, index) => (
                      <div key={index} className="flex justify-between items-center pb-2 border-b last:border-0">
                        <span className="text-muted-foreground">{formatPercentage(bracket.rate * 100, 0)} on {formatCurrency(bracket.income)}</span>
                        <span className="font-semibold">{formatCurrency(bracket.tax)}</span>
                      </div>
                    ))}
                    <div className="flex justify-between items-center pt-2">
                      <span className="font-semibold">Total Income Tax</span>
                      <span className="font-bold text-primary">{formatCurrency(results.incomeTax)}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-muted-foreground">Marginal Tax Rate</span>
                      <span className="font-semibold">{formatPercentage(results.marginalRate)}</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
                  ...
                  <CardContent className="space-y-3">
                    {results.childTaxCredit > 0 && (
                      <div className="flex justify-between items-center pb-2 border-b border-success/20">
                        ...
                        <span className="font-semibold text-success">{formatCurrency(results.childTaxCredit)}</span>
                      </div>
                    )}
                    {results.eitc > 0 && (
                      <div className="flex justify-between items-center pb-2 border-b border-success/20">
                        <span className="text-muted-foreground">Earned Income Tax Credit (EITC)</span>
                        <span className="font-semibold text-success">{formatCurrency(results.eitc)}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center pt-2">
                      <span className="font-semibold">Total Credits</span>
                      <span className="font-bold text-success">{formatCurrency(results.totalCredits)}</span>
                    </div>
                  </CardContent>
                </Card>
              )}
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <Alert className="max-w-5xl mx-auto">
            ...
            <AlertDescription className="text-base leading-relaxed mt-2">
              This calculator provides estimates based on the information you provide and current federal tax rates.
              Actual tax liability may vary based on your complete financial situation, additional deductions, credits,
              state taxes, and other factors. This tool should not be used for actual tax preparation or filing.
              For accurate tax preparation and to maximize your refund, <Link href="/contact" className="underline font-semibold">consult with a Tax Genius Pro certified tax professional</Link>.
            </AlertDescription>
          </Alert>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground mb-8 leading-relaxed">
              Get professional tax preparation from certified Tax Genius Pro tax professionals.
              We guarantee 100% accuracy and maximum refund.
            </p>
